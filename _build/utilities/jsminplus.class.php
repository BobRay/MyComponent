    ...
    private function Statement($x) {
        ...
        switch ($tt) {
            ...
            case KEYWORD_IF:
                ...
                $n->elsePart = $this->t->match(KEYWORD_ELSE)
                    ? $this->Statement($x)
                    : null;
                array_pop($x->stmtStack);
                return $n;
            ...
            case KEYWORD_FOR:
                ...
                if ($n2 && $this->t->match(KEYWORD_IN)) {
                    ...
                    $n->condition = $this->t->peek() == OP_SEMICOLON
                        ? null
                        : $this->Expression($x);
                    $this->t->mustMatch(OP_SEMICOLON);
                    $n->update = $this->t->peek() == OP_RIGHT_PAREN
                        ? null
                        : $this->Expression($x);
                }
...
if (! class_exists("JSCompilerContext")) {
                ...
                switch ($input[0]) {
                    ...
                    default:
                        ...
                        if (preg_match('/^[$\w]+/', $input, $match)) {
                            $tt = in_array($match[0], $this->keywords)
                                ? $match[0]
                                : TOKEN_IDENTIFIER;
                        } else
                            throw $this->newSyntaxError('Illegal token');
